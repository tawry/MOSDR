...
if __name__ == '__main__':
    import shutil
    import os
    # if os.path.exists('./twomodel_data3/twomodel_data3_L_Matrix'):   #检查当前目录下是否存在名为 'L_Matrix' 的文件夹
    #     shutil.rmtree('./twomodel_data3/twomodel_data3_L_Matrix/')   #如果存在会删除名为 'L_Matrix' 的文件夹及其内容
    # os.mkdir('./twomodel_data3/twomodel_data3_L_Matrix')             #创建一个新的名为 'L_Matrix' 的文件夹
    # if os.path.exists('./twomodel_data3/twomodel_data3_X_Matrix'):
    #     shutil.rmtree('./twomodel_data3/twomodel_data3_X_Matrix/')
    # os.mkdir('./twomodel_data3/twomodel_data3_X_Matrix')             #创建一个新的名为 'X_Matrix' 的文件夹
    data = pd.read_csv('../data_source/energydata_complete.csv')         #读取数据，并将其存储在变量 data 中
    init_data = data.iloc[:1000, 1:]        #选择了所有行（":" 表示选择所有行）和从第二列（索引为1）到最后一列的所有列，排除了 DataFrame 中的第一列
    ...
    for ratio,missing_row,missing_col in all_args:
        ...
        res_L.to_csv(f'./twomodel_data3/twomodel_data3_L_Matrix/L{result_index}.csv',index=False)

        to_cmp = pd.DataFrame(to_cmp, columns=init_data.columns)
        to_cmp.to_csv(f'./twomodel_data3/twomodel_data3_X_Matrix/X{result_index}.csv', index=False)
        result_index += 1
    ...
    plt.tight_layout()
    plt.savefig(f'./twomodel_data3/twomodel_data3_fig/fig2_{curr_time}.png', dpi=200)

    # result.to_csv(f'./twomodel_data3/twomodel_data3_result/result_{curr_time}.csv',index=False)
    '''
