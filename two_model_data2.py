...
def data_recovery(X, missing_cols, missing_rows, miu=0.01, lamda=0.01, beta=0.01, maxiter=100):
    ...
    L, S, G = test2(L, miu, lamda, beta, yita,maxiter)
    return L, S, G
...
def main(init_data, ratio=0.2, missing_row=0, missing_col=0):
    ...
    '''数据恢复'''
    L, S, G = data_recovery(X, missing_cols, missing_rows, miu, lamda, beta, maxiter)
    '''计算指标'''
...
if __name__ == '__main__':
    import shutil
    import os
    # if os.path.exists('./twomodel_data2/twomodel_data2_L_Matrix'):   #检查当前目录下是否存在名为 'L_Matrix' 的文件夹
    #     shutil.rmtree('./twomodel_data2/twomodel_data2_L_Matrix/')   #如果存在会删除名为 'L_Matrix' 的文件夹及其内容
    # os.mkdir('./twomodel_data2/twomodel_data2_L_Matrix')             #创建一个新的名为 'L_Matrix' 的文件夹
    # if os.path.exists('./twomodel_data2/twomodel_data2_X_Matrix'):
    #     shutil.rmtree('./twomodel_data2/twomodel_data2_X_Matrix/')
    # os.mkdir('./twomodel_data2/twomodel_data2_X_Matrix')             #创建一个新的名为 'X_Matrix' 的文件夹
    ...
    for ratio,missing_row,missing_col in all_args:
        ...
        res_L.to_csv(f'./twomodel_data2/twomodel_data2_L_Matrix/L{result_index}.csv',index=False)

        to_cmp = pd.DataFrame(to_cmp, columns=init_data.columns)
        to_cmp.to_csv(f'./twomodel_data2/twomodel_data2_X_Matrix/X{result_index}.csv', index=False)
        result_index += 1
    ...
    plt.tight_layout()
    plt.savefig(f'./twomodel_data2/twomodel_data2_fig/fig1_{curr_time}.png', dpi=200)

    sns.catplot(x="缺失列数", y="ERC", data=result, kind="point", hue="缺失行数", col="随机比例")
    plt.tight_layout()
    plt.savefig(f'./twomodel_data2/twomodel_data2_fig/fig2_{curr_time}.png', dpi=200)

    #result.to_csv(f'./twomodel_data2/twomodel_data2_result/result_{curr_time}.csv',index=False)
    '''
